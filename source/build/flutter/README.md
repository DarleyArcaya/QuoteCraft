...
* `source` - project identifier (lowercase, no spaces, i.e. "snake_case") used for bundle IDs and package names.
* `` - project description.
* `source` - executable or bundle name on disk.
* `QuoteCraft` - display name shown in window titles and about app dialogs.
* `com.tuempresa` - org name in reverse domain name notation, e.g. `com.mycompany.myproject`.
* `Darley Silot Arcaya` - the name of the company.
